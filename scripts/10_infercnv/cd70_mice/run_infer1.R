...
tumor_sample <- B_cell@meta.data  %>% filter(age == mos, genotype == "CD70-/-")  %>% 
                    pull(pool_id)  %>% unique()

print(length(tumor_sample))


for (i in tumor_sample[1]){

    ref_cell<- B_cell@meta.data  %>% filter(age == mos, genotype == "WT")  %>% rownames()
    ...
    infercnv_obj <- infercnv::run(infercnv_obj, cutoff=0.1,
                                  out_dir = file.path(paste0("./data/infercnv/inferCNV_out_new/cd70/", i, "/")),
